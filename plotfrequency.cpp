...
plotFrequency::plotFrequency(QWidget *parent) : QWidget(parent)
{
...
void plotFrequency::createPlot(QStringList curveName)
{
    ...
    customPlot1->xAxis->setLabel("Frequency(Hz)");
...
void plotFrequency::plotWidgetLayout()
{
...
//void plotFrequency::dataToPlot(QString plotName, QVector< QVector<QVector<double>>> vectorOfAllExcelDataVector, QVector<int> indexofExcelNum,QVector<int> indexofColNum)
//{
//    if("plot1"==plotName)
//    {
...
//            customPlot1->graph(i)->rescaleAxes(true);//坐标轴自适应
//        }
//        customPlot1->replot();
//    }
//    else if("plot2"==plotName)
//    {
////        for(int i=0; i<indexofExcelNum.size(); i++)
////        {
////            customPlot2->graph(i)->setData(vectorOfAllExcelDataVector.at(indexofExcelNum.at(i)).at(0),vectorOfAllExcelDataVector.at(indexofExcelNum.at(i)).at(indexofColNum.at(i)+1));
////            customPlot2->graph(i)->rescaleAxes(true);//坐标轴自适应
////        }
////        customPlot2->replot();
//    }
//}
...
//slots
void plotFrequency::selectionChanged()
{
...
void plotFrequency::contextMenuRequest(QPoint pos)
{
...
void plotFrequency::cursorMeasurement()
{
    cursorMeasuAct->setChecked(true);
...
void plotFrequency::arrowCursorSlotFunc()
{
    arrowCursorAct->setChecked(true);
...
void plotFrequency::my_mouseMove(QMouseEvent* event)
{
...
void plotFrequency::titleDoubleClick(QMouseEvent* event)
{
  Q_UNUSED(event)
...
void plotFrequency::datatoPlot(QVector<QVector<double>> vecofFfvec, QVector<QVector<double>> vecofAmplitudeVec)
{
    for(int i=0; i<vecofFfvec.size(); i++)
    {
        customPlot1->graph(i)->setData(vecofFfvec.at(i),vecofAmplitudeVec.at(i));
        customPlot1->rescaleAxes(true);//坐标轴自适应
    }
